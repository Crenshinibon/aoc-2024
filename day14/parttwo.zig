...
pub fn main() !void {
    ...
    outer: while (true) {
        ...
        for (0..DIM_Y) |y| {
            var bp_per_line = std.ArrayList(isize).init(allocator);

            for (bots.items) |b| {
                if (b.y == y) {
                    try bp_per_line.append(b.x);
                }
            }

            std.mem.sort(isize, bp_per_line.items, {}, comptime std.sort.asc(isize));

            //look for consecutive
            var offset: isize = 0;
            var counter: usize = 0;
            for (bp_per_line.items) |p| {
                if (offset == 0) {
                    offset = p;
                } else {
                    if (offset + 1 == p or offset == p) {
                        counter += 1;
                        offset = p;
                    } else {
                        offset = p;
            ...
            if (counter > 2 and bp_per_line.items.len > 10) {
                std.debug.print("counter: {} - {}\n", .{ counter, bp_per_line.items.len });

                for (bp_per_line.items) |i| {
                    std.debug.print("{}-", .{i});
                }
                std.debug.print("\n", .{});
            }
